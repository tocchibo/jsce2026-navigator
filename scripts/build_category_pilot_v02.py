...
FIELD_WEIGHTS = {"title": 5, "keywords": 6, "session": 2, "summary": 3}
THRESHOLDS = {"domain": 5, "phase": 5, "method": 5, "material": 5, "issue": 5}

RULES: dict[str, dict[str, tuple[str, ...]]] = {
    "domain": {
        "bridge": (r"橋梁|道路橋|鉄道橋|橋りょう|床版|橋脚|橋台|支承|斜張橋|吊り橋|PC桁|鋼桁|合成桁|鈑桁|主桁",),
        "road_pavement": (r"道路|高速道路|舗装|路盤|路床|交差点|交通流|通行止め|ETC|BPR関数",),
        "railway": (r"鉄道|軌道|新幹線|線路|列車|レール|ホーム柵|走行安全性",),
        "tunnel_underground": (r"トンネル|シールド|NATM|セグメント|坑門|覆工|地下空間",),
        "ground_foundation_slope": (r"地盤|地山|斜面|盛土|土質|土層|液状化|杭|基礎(?!的|物性|研究|実験)|圧密|ベントナイト|土留め|地中連続壁|カルバート|ケーソン",),
        "river_watershed": (r"河川|河道|流域|洪水|氾濫|流出|砂防|治山ダム|高水敷|流域治水|掃流|河床",),
        "coast_port_ocean": (r"海岸|港湾|岸壁|漁港|海洋|海底|沿岸|アマモ|海水|洋上",),
        "dam_reservoir": (r"ダム湖|貯水池|ダム運用|水力発電ダム",),
        "water_sewerage": (r"上水|下水|浄水|用排水|水処理|汚泥|フロック形成池|メタン発酵|管路|不断水",),
        "atmosphere_climate": (r"降雨|降雪|積雪|気温|気候|降水|線状降水帯|熱環境",),
        "urban_transport": (r"都市|地域計画|公共交通|人流|交通結節|交通行動|居住|歩行者|避難支援|交通規制",),
        "structural_system": (r"テンセグリティ|構造物|ボックスカルバート|送電鉄塔|浮体|接合部|継手|ボルト|溶接|耐荷力|防護柵|石垣",),
        "construction_materials": (r"コンクリート|セメント|モルタル|アスファルト|鋼材|鋼素地|FRP|ジオポリマー|スラグ|固化材|材料",),
        "construction_site": (r"建設現場|建設業|作業所|施工管理|出来形管理|工事安全|施工現場|現場における",),
        "environment_ecosystem": (r"生態|生物多様性|環境DNA|自然共生|グリーンインフラ|植被|アマモ場|都市熱環境|環境保全",),
        "waste_contamination": (r"廃棄物|汚染|焼却施設|放射性|マイクロプラスチック|塩素化エチレン",),
        "energy_industrial": (r"発電|風力|送電|エネルギー|CCUS|CO2|カーボンニュートラル",),
        "space_lunar": (r"宇宙|月面|レゴリス",),
        "civil_systems": (r"社会基盤|インフラ|建設DX|組織的運用|マネジメント",),
        "education_society": (r"教育|人材|人財|リスキリング|リテラシー|技術継承|DEI|合意形成",),
    },
    "phase": {
        "policy_planning": (r"計画|政策|構想|ガイドライン|候補案|立地特性|意思決定",),
        "survey_measurement": (r"調査|計測|測定|観測|モニタリング|検層|サウンディング|推定",),
        "design_analysis": (r"設計|照査|解析|シミュレーション|予測|推計|算定|試算|モデル|FEM|有限要素",),
        "performance_evaluation": (r"性能|評価|特性|挙動|強度|耐荷|実験|試験|影響|機構|再現|検証|物性|安定性|効率|変化|基礎研究|実験的検討|原因.*究明|比較|分析",),
        "technology_development": (r"システム|アプリケーション|手法の開発|技術.*開発|モデルの提案|高度化",),
        "material_development": (r"材料.*開発|工法.*開発|固化材|配合|混和|ジオポリマー|相溶性|プレキャスト化",),
        "construction": (r"施工|工事|掘削|築造|製造|打設|解体",),
        "quality_control": (r"品質管理|検査|出来形|単位水量|施工管理|品質確認",),
        "inspection_diagnosis": (r"点検|診断|異常検知|健全性|損傷評価|変状|打音",),
        "maintenance_repair": (r"維持管理|補修|耐震補強|補強工|既設.*補強|床版補強|インフラ保全|維持保全|修復|取替|長期保全|機能の回復",),
        ...
        "education_consensus": (r"教育|人材育成|人財育成|リスキリング|リテラシー|技術継承|合意形成",),
    },
    "method": {
        ...
        for field_name, text in fields.items():
            for pattern in patterns:
                if re.search(pattern, text, flags=re.IGNORECASE):
                    scores[label_id] += FIELD_WEIGHTS[field_name]
...
def classify(item: dict[str, object], axis_by_id: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for axis_id, axis in axis_by_id.items():
        ...
        if len(candidates) > max_items:
            review_reasons.append(f"{axis_id}:tag_limit")
    ...
    required_scores = [
        score_evidence[axis_id][labels[axis_id][0]]
        for axis_id, axis in axis_by_id.items()
...
def main() -> int:
    ...
    presentations = [classify(item, axis_by_id) for item in source["presentations"]]
    output = {
        ...
        "classification_method": "weighted explicit evidence rules v0.2",
